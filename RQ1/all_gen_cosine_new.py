...
#     for year in year_list:
    print("\n\n")
    print("-----"*6)
    a=pd.read_excel("Claim_Norm_chatgpt/" + n1 + ".xlsx")
    a["date_obj"] = a["date"].map(lambda d:datetime.datetime.strptime(d, str_format))
    l1 = a.shape[0]
    b=pd.read_excel("Claim_Norm_chatgpt/" + n2 + ".xlsx")
    b["date_obj"] =  b["date"].map(lambda d:datetime.datetime.strptime(d, str_format))
    c=pd.read_excel("Claim_Norm_chatgpt/" + n3 + ".xlsx")
    c["date_obj"] = c["date"].map(lambda d:datetime.datetime.strptime(d, str_format))
    print(n1,n2,n3,a.shape,b.shape,c.shape)
    sys.exit()
    sys.stdout.flush()
    with open("Claim_Norm_chatgpt/"+n1+"_"+n2+"_"+json_fin+"_PARALLEL.json", "r") as f:
        f1 = json.load(f)
    with open("Claim_Norm_chatgpt/"+n1+"_"+n3+"_"+json_fin+"_PARALLEL.json", "r") as f:
        f2 = json.load(f)
    ...
    with open("Claim_Norm_chatgpt/"+n1+"_"+json_fin+"_all_cosine_NEW.json","w") as f:
