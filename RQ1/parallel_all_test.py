...
#     for year in year_list:
    print("\n\n")
    print("-----"*6)
    a=pd.read_excel("Claim_Norm_chatgpt/" + n1 + "_emb_all.xlsx")
    b=pd.read_excel("Claim_Norm_chatgpt/" + n2 + "_emb_all.xlsx")
...
def get_cosine_15_parallel(a,b,n1,n2,json_fin):
    ...
    cosine_list = Parallel(n_jobs=num_cores, backend="threading")(delayed(process_item)(i, emb_n1_list[i], j, emb_n2_list[j], n1, n2) for i in range(la) for j in range(lb))
    with open(os.path.join("Claim_Norm_chatgpt", n1 + "_" + n2 + "_" + json_fin + "_PARALLEL.json"), "w") as f:
